...
<?php
// most of the code has been adapted from this open source 
//http://www.planetphp.co.uk/free-php-booking-slots-calendar/
...
public $booking_start_time          = "10:00";			// The time of the first slot in 24 hour H:M format  
...
public $cost_per_slot				= 20;			// The cost per slot
...
function make_booking_array($year, $month, $j = 0) { 
	...
	$stmt = $this->link->prepare("SELECT username, date, start FROM bookings WHERE date LIKE  CONCAT(?, '-', ?, '%')"); 
		...
		<div id='selected_slots'></div>		
			...
			<div id='basket_details'>
				...
				<form method='post' action='book_slots.php'>
					...
					<input name='descriptions' id='name' type='text' class='text_box'>
